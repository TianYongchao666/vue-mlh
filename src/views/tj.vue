<template>
  <div>
    ...
    <div class="NewDaily">
      <div class="NewDaily-one" v-for="a in nav " :key="a.id">
        <a href="#">
          <div class="NewDaily-one-top">
            <p>{{a.main_title}}</p>
            <p class="title">{{a.sub_title}}</p>
          </div>

          <div class="NewDaily-one-bottom">
            <div class="image-one">
              <img :src="a.products[0].picUrl" alt />
              <p>{{a.products[0].tag}}</p>
            </div>
            <div class="image-two">
              <img :src="a.products[1].picUrl" alt />
              <p>{{a.products[0].tag}}</p>
            </div>
          </div>
        </a>
...
export default {
  ...
  created() {
    ...
    axios
      ...
      .then(res => {
        this.nav = res.data.show2;
      });
    axios
      .get(
        "http://www.mei.com/appapi/home/eventForH5?params=%7B%7D&timestamp=1592363603336&summary=b82bb5b68fb656cc62a114b696151337&platform_code=H5"
      )
      .then(res => {
...
.flooter-three {
  ...
  img {
    width: 36.389vw;
    height: 5.556vw;
    margin: 2.778vw 8.333vw;
    //  flex: 1;
  }
  .flooter-three-bottom {
    display: flex;
    justify-content: space-between;
    align-items: center;
    // flex: 1;
